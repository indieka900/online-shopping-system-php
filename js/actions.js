$(document).ready(function(){
	...
	*/
    ...
    $("body").delegate(".remove","click",function(event){
		...
		var qty = remove.find(".qty").val(); 

		console.log('Here is quantity ',qty)
		
		$.ajax({
			url     :   "action.php",
			method  :   "POST",
			data    :   {removeItemFromCart: 1, rid: remove_id, qt_y: qty}, // Include qty in the data object
			success :   function(data){
	...
	*/
	...
	$("body").delegate(".update","click",function(event){
		...
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{updateCartItem:1,update_id:update_id,q_ty:qty},
			success	:	function(data){
